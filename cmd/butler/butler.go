      ...
      //      os.Exit(1)
        ...
        //go func(c net.Conn) {
          //  req := make([]byte, 1024)
            ...
            //reqBytes, err := c.Read(req)
            //if err != nil {
              //  fmt.Println("Error reading request:", err.Error())
            //}

           // rqln, hdrs, _ := parseRequest(req[:reqBytes])
            //_, target, _ := parseRequestLine(rqln)
            
           // var msg string
            
            //if string(target) == "/" {
              //  msg = "HTTP/1.1  200 OK\r\n\r\n"
            //} else if strings.Contains(string(target), "/echo/") {
              //  echo := bytes.Split(target, []byte("/"))[2]
                //fmt.Println(string(echo))
                //length := strconv.Itoa(len(echo))
                //fmt.Println(length)
                //msg = "HTTP/1.1 200 OK\r\n" + "Content-Type: text/plain\r\nContent-Length: " + length + "\r\n\r\n" + string(echo)
                //fmt.Println(msg)
            //} else if string(target) == ("/user-agent") {
              //  usrAgent, _ := getHeader(hdrs,"User-Agent")
                //length := strconv.Itoa(len(usrAgent))
                //msg = "HTTP/1.1 200 OK\r\n" + "Content-Type: text/plain\r\nContent-Length: " + length + "\r\n\r\n" + string(usrAgent)
            //} else {
